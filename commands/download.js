...
cmd({
            ...
            category: "downloader",
            filename: __filename,
            use: '<Hii,this is Raizel>',
        },
        async(Void, citel, text) => {
            if (!text && !citel.quoted)
            { 
              citel.reply('*مثال .انطق رايزل جورج*');
             return;
            }
            if (!text) 
            {
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply(`.نغمه back in black`)
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply(`جيب رابط ${tlang().greet}`);
            ...
            const result4 = `*تحميل من ميديافاير*
*الأسم* : ${baby1[0].nama}
*الحجم* : ${baby1[0].size}
*قلد* : ${baby1[0].mime}
*الرابط* : ${baby1[0].link}`;
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) {
                        citel.reply(`.اغنيه Back in black*`);
                        return ;
                       }
            let yts = require("secktor-pack")
            let search = await yts(text)
            listSerch = []
            teskd = `النتيجه ${text}.\n_+ ${search.all.length} المزيد من النتائج._`
            for (let i of search.all) {
                listSerch.push({
                    title: i.title,
            ...
            const sections = [
                ...
                {
                    title: "موضوع البحث🔍" + search.all.length,
                    rows: listSerch
                }
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            if (!text) {
                citel.reply(`❌الرجاء ارسال رابط فيديو يوتيوب`);
                return;
            }
            try {
                let urlYt = text;
                if (!urlYt.startsWith("http")) return citel.reply(`❌ جيب رابط يوتيوب!`);
                let infoYt = await ytdl.getInfo(urlYt);
                if (infoYt.videoDetails.lengthSeconds >= videotime) return citel.reply(`❌ الملف كبير لايمكنني تحميله!`);
                ...
                if (fileSizeInMegabytes <= dlsize) {
                    ...
                    let buttonMessage = {
                        ...
                        caption: ` ⿻ العنوان : ${titleYt}\n ⿻ الحجم : ${fileSizeInMegabytes} MB`,
                        headerType: 4,
                        contextInfo: {
                            externalAdReply: {
                                title: titleYt,
